...
def write_results(ARG, ARG_seq, ARG_path, g1, left_seq_list, right_seq_list, all_paths, output_path, gene_cov, radius):
    ...
    if not os.path.exists(f"result_contexts_cov_{gene_cov}_radius_{radius}/"):
        os.makedirs(f"result_contexts_cov_{gene_cov}_radius_{radius}/")

    with open(f"result_contexts_cov_{gene_cov}_radius_{radius}/left_seq.fasta", "a") as file:
        for idx, seq in enumerate(left_seq_list):
            header = f">{ARG}_{'_'.join(ARG_path)}_{idx}"
            file.write(header + "\n" + seq + "\n")

    with open(f"result_contexts_cov_{gene_cov}_radius_{radius}/right_seq.fasta", "a") as file:
        for idx, seq in enumerate(right_seq_list):
            header = f">{ARG}_{'_'.join(ARG_path)}_{idx}"
            file.write(header + "\n" + seq + "\n")
    
    with open(f"result_contexts_cov_{gene_cov}_radius_{radius}/contexts.txt", 'a') as file:
        for idx, path in enumerate(all_paths):
            header = f">{ARG}_{'_'.join(ARG_path)}_{idx}"
            file.write(header + "\n" + "->".join(path) + "\n")
    
    with open(f"result_contexts_cov_{gene_cov}_radius_{radius}/ARG_seq.fasta", 'a') as file:
        header = f">{ARG}_{'_'.join(ARG_path)}"
        file.write(header + "\n" + ARG_seq + "\n")

    with open(f"result_contexts_cov_{gene_cov}_radius_{radius}/whole_context_with_length.fasta", "a") as whole_file:
...
def extract_merge_seq(all_paths, ARG, start_node, end_node, g1, ARG_dict, ARG_path, radius, overlap, min_context_len, output, gene_cov):
    left_list = []
    right_list = []
    ...
    write_results(ARG, ARG_seq, ARG_path, g1, left_seq_list, right_seq_list, all_paths, output, gene_cov, radius)


def extract_ARG_contexts(g1, ARG_dict, ARG_path_dict, radius, overlap, min_context_len, output, gene_cov):
    mapping = {"+": "-", "-": "+"}
    for ARG, paths in ARG_path_dict.items():
        for path in paths:
            ...
            extract_merge_seq(all_paths, ARG, start_node, end_node, g1, ARG_dict, path, radius, overlap, min_context_len, output, gene_cov)
